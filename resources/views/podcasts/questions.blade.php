        ...
        <tbody>
            @foreach($lesson->questions as $q)
                <tr>
                    ...
                    <td>{{ $q->question }}</td>
                    <td>
                        <a href="{{ route('discussion_questions.edit', $q->id) }}" class="btn btn-sm btn-warning">Edit</a>
                        <form method="POST" action="{{ route('discussion_questions.destroy', $q->id) }}" style="display:inline">
                            @csrf @method('DELETE')
